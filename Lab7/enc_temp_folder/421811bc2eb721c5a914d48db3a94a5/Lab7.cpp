...
namespace SpaceTask4 {
	...
	*/	void PrintElement() {
	...
	class TreeIterator {
	private:
		TreeElement* stack[100];
		int stackCurrPos = 0;

	public:
		TreeIterator(TreeElement* root)
		{
			moveLeft(root);
		}

		void moveLeft(TreeElement* current)
		{
			while (current) {
				stack[stackCurrPos++] = current;
	...
	//			throw "No such element Exists";
			...
			TreeElement* current = stack[--stackCurrPos];
	/*			traversal.top();
			traversal.pop();*/
	...
	int mainTask4()
	{
		...
		(*(*obj1.GetRight()).GetLeft()).SetLeft(new TreeElement(100));
		(*(*obj1.GetRight()).GetLeft()).SetRight(new TreeElement(120));

		obj1.PrintElement();
		cout << endl;

		TreeIterator iter = TreeIterator(&obj1);
		while (iter.hasNext()) {
			TreeElement* e = iter.next();
			e->PrintValue();
			cout << endl;
		}
		//while
